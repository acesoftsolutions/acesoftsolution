...
/**
 * Icons are pulled from the Simple Icons CDN (https://cdn.simpleicons.org/<slug>/<hex>),
 * which serves real, recognizable brand marks as lightweight SVGs — no extra npm
 * dependency required. Each technology also carries an `accent` used for its glow,
 * gradient border, and hover state so every card feels tied to its actual brand.
 */
const categories = [
  { id: "ai", label: "AI & ML", icon: Brain, accent: "#7C3AED" },
  { id: "frontend", label: "Frontend", icon: Monitor, accent: "#0284C7" },
...
const technologies = [
  ...
  { name: "LlamaIndex", category: "ai", slug: "llama", accent: "5C5CFF" },
  { name: "Vector DB", category: "ai", slug: "qdrant", accent: "DC244C" },

  // Frontend
  { name: "React", category: "frontend", slug: "react", accent: "087EA4" },
  ...
  { name: "React Native", category: "mobile", slug: "react", accent: "087EA4" },
  ...
  { name: "CI/CD", category: "cloud", slug: "githubactions", accent: "2088FF" },
        ...
        <AnimatedSection>
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="mx-auto mt-6 max-w-3xl text-lg leading-relaxed text-slate-600">
              ...
              platforms, advanced AI technologies, and scalable architectures
              to build secure, future-ready, and high-performance digital
              products.
            </p>
          </div>
        </AnimatedSection>
            ...
            {categories.map((category) => {
              ...
              return (
                <button
                  ...
                  className={`group relative flex items-center gap-2 rounded-2xl px-5 py-3 text-sm font-medium transition-all duration-300 ${
                    isActive
                      ? "text-slate-900"
            ...
            >
              {filteredTechnologies.map((tech, index) => (
                <AnimatedSection key={tech.name} delay={index * 0.03}>
                  <motion.div
                    whileHover={{ y: -6 }}
                    className="group relative flex h-full min-h-[160px] flex-col items-center justify-center overflow-hidden rounded-[28px] border border-slate-200 bg-white p-6 text-center shadow-sm transition-all duration-300 hover:border-slate-300 hover:shadow-xl"
                  >
                    {/* hover glow tinted to the brand color */}
                    <div
                      className="pointer-events-none absolute -top-10 left-1/2 h-32 w-32 -translate-x-1/2 rounded-full opacity-0 blur-3xl transition-opacity duration-500 group-hover:opacity-[0.12]"
                      style={{ backgroundColor: `#${tech.accent}` }}
                    />

                    <div className="relative flex h-16 w-16 items-center justify-center rounded-2xl bg-slate-50 p-3.5 transition-all duration-300 group-hover:scale-110 group-hover:bg-white group-hover:shadow-md">
                      <img
                        ...
                        onError={(e) => {
                          // fallback to a neutral mark if a slug isn't on Simple Icons
                          e.currentTarget.src = `https://cdn.simpleicons.org/${tech.slug}/64748B`;
                        }}
                      />
                    </div>

                    <h3 className="relative mt-5 font-semibold text-slate-900">
                      {tech.name}
                    </h3>
                  </motion.div>
                </AnimatedSection>
